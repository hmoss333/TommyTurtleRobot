...
public class Loops2 : MonoBehaviour
{
    ...
    void checkCorrect()
    {
        try
        {
            if (movement[0].Equals("Begin Loop") && movement[1].Equals("Spin") && (movement[2].Equals("Grow") || movement[2].Equals("Dance")) && movement[3].Equals(startFormat + "End Loop" + endFormat))
            {
                displayWinScreen();
            }
            else
